module Sovren
  class Client
    ...
    def parse(file, options = {})
      response = connection.call(:parse_resume) do |c|
        c.message('request' => options_for_request(file, options))
      end

      result = response.body[:parse_resume_response][:parse_resume_result]

      if result[:code] == 'Success'
        Resume.parse result[:xml]
      else
        fail Sovren::Error, result
      end
    end

    private

    def options_for_request(file, options = {})
      {
